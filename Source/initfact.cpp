...
#endif
{
 ...
 VALUETYPE Vk,thk,Vm,thm,Xc,Xl,Ptcsc,Qtcsck,Qtcscm,Be,alpha,Itcsc,delta_t;
  ...
  /* --------------- Initialize the TCSC system variables ---------------------- */
    ...
    if(Stcsc==0) Stcsc=TCSCptr->TCSC_base=Sn;
    Vn=TCSCptr->From->KV;
    ...
    /* } else Be=1.0/Xc-(2.0*PI-2.0*alpha+sin(2.0*alpha))/(PI*Xl); */
      ...
      Be = PI*cos(Kf*(-PI+alpha))*(pow(Kf,4.0)-2.0*Kf*Kf+1.0)/Xc/(-PI*pow(Kf,4.0)*
    ...
    } else Ptcsc=-Vk*Vm*Be*sin(thk-thm);
    ...
    if(!strcmp(TCSCptr->Cont,"I")) {
      ...
      if (Ptcsc >= 0) Itcsc=(sqrt(Ptcsc*Ptcsc+Qtcsck*Qtcsck))/Vk;
      else Itcsc=-(sqrt(Ptcsc*Ptcsc+Qtcsck*Qtcsck))/Vk;
    }
    if(!strcmp(TCSCptr->Cont,"D")) {
      delta_t=Ctrl*K3;
      TCSCptr->Control = delta_t;
    } else delta_t=thk-thm;
    TCSCptr->Ptcsc=Ptcsc;
