  ...
  for (ACptr = dataPtr->ACbus; ACptr != nullptr; ACptr = ACptr->Next)
  {
      ...
      if (strpbrk(ACptr->Type, "V,Q,S,G,Z"))
      {
        ...
        { /*Gen. Model BEGIN */
          ...
          if (strpbrk(ACptr->cont, "I"))
            ...
            JacElement(Mptr, i + 1, j + 11, 0.);
          if (strpbrk(ACptr->cont, "E"))
            JacElement(Mptr, i + 1, j + 1, 1.);
          else
            JacElement(Mptr, i + 1, j + 1, 0.);
        } /* Gen. Model  END */
      }
      ...
      for (ELptr = BEptr->Area->Elem; ELptr != nullptr; ELptr = ELptr->Next)
      {
        ...
        if (flagJ)
        {
          ...
          if (From->Cont != nullptr)
            JacElement(Mptr, i, j + 1, dPiiv);
          else if (flagH && strpbrk(From->Type, "L"))
            JacElement(Mptr, i, Mptr->n1, dPiiv);
          else
            JacElement(Mptr, i, j + 1, 0.);
          j = ACvar[To->N];
        ...
        if (strpbrk(Eptr->Type, "PQMN"))
        {
          ...
          else
          {
            if (flagJ)
            {
              ...
              if (From->Cont != nullptr)
                JacElement(Mptr, i, j + 1, dQiiv);
              else if (flagH && strpbrk(From->Type, "L"))
                JacElement(Mptr, i, Mptr->n1, dQiiv);
              else
                JacElement(Mptr, i, j + 1, 0.);
              j = ACvar[To->N];
              if (!strpbrk(To->Type, "S"))
                JacElement(Mptr, i, j, dQijd);
              if (To->Cont != nullptr)
                JacElement(Mptr, i, j + 1, dQijv);
              else if (flagH && strpbrk(To->Type, "L"))
                JacElement(Mptr, i, Mptr->n1, dQijv);
              else
                JacElement(Mptr, i, j + 1, 0.);
            }
            if (flagF)
    ...
    if (ACptr->Gen != nullptr)
    {
      ...
      if (flagJ)
      {
        ...
        if (strpbrk(ACptr->cont, "I"))
          JacElement(Mptr, i + 2, i + 11, 1.);
        else
          JacElement(Mptr, i + 2, i + 11, 0.);
        if (strpbrk(ACptr->cont, "E"))
          JacElement(Mptr, i + 2, i + 1, 1.);
        else
          JacElement(Mptr, i + 2, i + 1, 0.);
        ...
        /* df3/dx  */
        if (strpbrk(ACptr->cont, "E"))
          JacElement(Mptr, i + 3, i + 1, 0.);
        else
          JacElement(Mptr, i + 3, i + 1, 1.);
        ...
        j = ACvar[ACptr->N];
        if (ACptr->Cont != nullptr)
          JacElement(Mptr, i + 9, j + 1, -cos(di));
        else
        ...
        j = ACvar[ACptr->N];
        if (ACptr->Cont != nullptr)
          JacElement(Mptr, i + 10, j + 1, -sin(di));
        else
        ...
        if (strpbrk(ACptr->cont, "I"))
          JacElement(Mptr, i + 11, i + 11, 0.);
        else
          JacElement(Mptr, i + 11, i + 11, 2. * Ia);
      }
    }
  }
