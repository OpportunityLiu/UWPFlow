...
extern bool Acont, PQcont, QRcont, Rcont, PQlim, Tlim, Qlim, Ilim, Elim, Vlim,
    Zlim, Xlim, flagH, flagPoC, flagL, flagR, flagBS, flagPgMax, flagSmax;
...
bool CheckRlimits(void) {
  ...
  for (ACptr = dataPtr->ACbus; ACptr != nullptr; ACptr = ACptr->Next) {
    for (ELptr = ACptr->Reg; ELptr != nullptr; ELptr = ELptr->Next) {
      Eptr = ELptr->Eptr;
      if (Tlim && !strcmp(Eptr->Type, "R") &&
          (Eptr->Tap <= 1 / Eptr->Tmax + 0.00001 ||
           Eptr->Tap >= 1 / Eptr->Tmin - 0.00001)) {
        flag = true;
        i = j = 0;
        NregV--;
        if (Eptr->Tap <= 1 / Eptr->Tmax + 0.00001) {
        ...
        for (Rptr = ACptr->Reg; Rptr != nullptr; Rptr = Rptr->Next) {
          if (!strcmp(Rptr->Eptr->Type, "R"))
            i++;
          else if (!strcmp(Rptr->Eptr->Type, "RV"))
            j++;
        }
        if (i == 1 && j == 0) {
          strcpy(ACptr->Type, "B");
        ...
        fprintf(stderr, "***Warning: %1s Reg. transf. from %d %s to %d %s\n",
                Eptr->Ctype, Eptr->From->Num, Eptr->From->Name, Eptr->To->Num,
                Eptr->To->Name);
        fprintf(
            stderr,
            "            has been changed to a fixed tap transf. at t%3s.\n",
            str);
      } else if (Rcont && !strcmp(Eptr->Type, "RV") &&
                 (ACptr->V >= Eptr->Max || ACptr->V <= Eptr->Min)) {
        flag = true;
        strcpy(Eptr->Type, "R");
        if (ACptr->V >= Eptr->Max) {
          ACptr->V = Eptr->Max;
        ...
        fprintf(
            stderr,
            "            has been changed to an LTC at V%3s for bus %d %s.\n",
            str, ACptr->Num, ACptr->Name);
      } else if (Tlim && !strcmp(Eptr->Type, "RP") &&
                 (Eptr->Ang >= Eptr->Tmax || Eptr->Ang <= Eptr->Tmin)) {
        flag = true;
        NregPQ--;
        strcpy(Eptr->Type, "TP");
        if (Eptr->Ang >= Eptr->Tmax) {
          Eptr->Ang = Eptr->Tmax;
          strcpy(str, "max");
        } else {
          Eptr->Ang = Eptr->Tmin;
        ...
        fprintf(
            stderr,
            "            has been changed to a fixed angle transf. at a%3s.\n",
            str);
      } else if (Tlim && !strcmp(Eptr->Type, "RQ") &&
                 (Eptr->Tap <= 1 / Eptr->Tmax + 0.00001 ||
                  Eptr->Tap >= 1 / Eptr->Tmin - 0.00001)) {
        flag = true;
        NregPQ--;
        ...
        fprintf(stderr, "***Warning: %1s Reg. transf. from %d %s to %d %s\n",
                ...
                Eptr->To->Name);
        fprintf(stderr,
                "            has been changed to a fixed tap at t%3s.\n", str);
      } else if (PQcont && PQlim && strpbrk(Eptr->Type, "M") &&
                 (Eptr->Cvar >= Eptr->Max || Eptr->Cvar <= Eptr->Min)) {
        flag = true;
        strcpy(Eptr->Type, "RP");
        if (Eptr->Cvar >= Eptr->Max) {
          Eptr->Cvar = Eptr->Max;
          strcpy(str, "max");
        } else {
          Eptr->Cvar = Eptr->Min;
        ...
        fprintf(stderr, "***Warning: %1s Reg. transf. from %d %s to %d %s\n",
                ...
                Eptr->To->Name);
        fprintf(stderr,
                "            has been changed to control %1s at P%3s.\n",
                Eptr->Ctype, str);
      } else if (PQcont && PQlim && strpbrk(Eptr->Type, "N") &&
                 (Eptr->Cvar >= Eptr->Max || Eptr->Cvar <= Eptr->Min)) {
        flag = true;
        strcpy(Eptr->Type, "RQ");
        if (Eptr->Cvar >= Eptr->Max) {
        ...
        fprintf(stderr, "***Warning: %1s Reg. transf. from %d %s to %d %s\n",
                ...
                Eptr->To->Name);
        fprintf(stderr,
                "            has been changed to control %1s at Q%3s.\n",
                Eptr->Ctype, str);
      }
    }
  }
...
bool CheckVlimits(void) {
  ...
  if (ExistParameter('G'))
    Recover = false;
  if (Vlim || Elim || Ilim || Xlim)
    for (ACptr = dataPtr->ACbus; ACptr != nullptr; ACptr = ACptr->Next) {
      if (strpbrk(ACptr->Type, "V")) {
        if (Vlim && strpbrk(ACptr->cont, "Q") &&
            (ACptr->V >= ACptr->Vmax || ACptr->V <= ACptr->Vmin)) {
          flag = true;
          Nvolt++;
          if (ACptr->V >= ACptr->Vmax) {
            ACptr->V = ACptr->Vmax;
            strcpy(str, "max");
          } else {
            ACptr->V = ACptr->Vmin;
          ...
          fprintf(stderr,
                  "***Warning: Generator %d %s will start controlling the "
                  "voltage\n",
          ...
          if (Ilim && strpbrk(ACptr->cont, "Q") &&
            ...
            fprintf(stderr, "***Warning: Generator %d %s has recovered\n",
                    ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            Q control as Ia is again within limits.\n");
            ...
            fprintf(stderr, "***Warning: Generator %d %s has recovered\n",
                    ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            Q control as Eq is again within limits.\n");
          } else if (strpbrk(ACptr->cont, "V") &&
                     ((Ilim && ACptr->Gen->Ia >= ACptr->Gen->IaMax) ||
                      (Elim && (ACptr->Gen->Eq >= ACptr->Gen->EqMax ||
                                ACptr->Gen->Eq <= ACptr->Gen->EqMin)))) {
            fprintf(stderr,
                    "***Error: Generator %d %s it's already at a V limit.\n",
                    ACptr->Num, ACptr->Name);
            ...
            exit(1);
                     ...
                     ((Vlim &&
                      ...
                      (Elim && (ACptr->Gen->Eq >= ACptr->Gen->EqMax ||
                                ACptr->Gen->Eq <= ACptr->Gen->EqMin)))) {
            fprintf(stderr,
                    "***Error: Generator %d %s it's already at an Ia limit.\n",
                    ACptr->Num, ACptr->Name);
            ...
            exit(1);
                     ...
                     ((Ilim && ACptr->Gen->Ia >= ACptr->Gen->IaMax) ||
                      (Vlim &&
                       (ACptr->V >= ACptr->Vmax || ACptr->V <= ACptr->Vmin)))) {
            fprintf(stderr,
                    "***Error: Generator %d %s it's already at an Eq limit.\n",
                    ACptr->Num, ACptr->Name);
        ...
        if (Xlim && strpbrk(ACptr->cont, "X") && ACptr->V <= ACptr->Vmin) {
          ...
          if (ACptr->Cont->Bx[0] > 0) {
            ...
            if (i <= ACptr->Cont->steps) {
              ...
              fprintf(stderr,
                      "***Warning: Reactance-controlled bus %d %s requires a "
                      "%d st/nd/rd/th\n",
                      ACptr->Num, ACptr->Name, i);
              fprintf(stderr,
                      "            increment of %4.0lf MVar due to V_min "
                      "problems.\n",
                      Sn * ACptr->Cont->Bx[i]);
            }
          } else {
            ...
            if (i >= 1) {
              ...
              fprintf(stderr,
                      ...
                      ACptr->Num, ACptr->Name, i);
              fprintf(stderr,
                      "            reduction of %4.0lf MVar due to V_min "
                      "problems.\n",
                      -Sn * ACptr->Cont->Bx[i]);
              ACptr->Cont->step--;
            }
          ...
          if (ACptr->Cont->step > ACptr->Cont->steps || ACptr->Cont->step < 1) {
            ...
            fprintf(
                stderr,
                "***Warning: Reactance-controlled bus %d %s has run out of \n",
                ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            reactance support due to V_min problems.\n");
          }
          ...
          if (ACptr->Cont->Bx[0] > 0) {
            ...
            if (i >= 1) {
              ...
              fprintf(stderr,
                      ...
                      ACptr->Num, ACptr->Name, i);
              fprintf(stderr,
                      "            reduction of %4.0lf MVar due to V_max "
                      "problems.\n",
                      Sn * ACptr->Cont->Bx[i]);
              ACptr->Cont->step--;
            }
            ...
            if (i <= ACptr->Cont->steps) {
              ...
              fprintf(stderr,
                      ...
                      ACptr->Num, ACptr->Name, i);
              fprintf(stderr,
                      "            increment of %4.0lf MVar due to V_max "
                      "problems.\n",
                      -Sn * ACptr->Cont->Bx[i]);
            }
          }
          if (ACptr->Cont->step > ACptr->Cont->steps || ACptr->Cont->step < 1) {
            ...
            fprintf(
                ...
                ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            reactance support due to V_max problems.\n");
          }
        } else if (Xlim && Recover && strpbrk(ACptr->cont, "M") &&
                   ACptr->V <= ACptr->Vmax) {
          flag = true;
          NXvolt++;
          strcpy(ACptr->cont, "X");
          fprintf(stderr,
                  "***Warning: Reactance-controlled bus %d %s has recovered \n",
                  ACptr->Num, ACptr->Name);
...
bool CheckQlimits(void) {
  ...
  if (ExistParameter('G'))
    Recover = false;
  if (Qlim || Elim || Ilim || Zlim)
    for (ACptr = dataPtr->ACbus; ACptr != nullptr; ACptr = ACptr->Next) {
      if (ACptr->flagPgMax == 1) {
        fprintf(stderr,
                "***Warning: Generating bus %d %s has reached the maximum P/S "
                "limit;\n",
                ACptr->Num, ACptr->Name);
        fprintf(stderr,
                "            hence, Pg will be fixed at its max. MW limit -> "
                "%lf.\n",
                ACptr->Pmax * Sn);
        ACptr->flagPgMax++;
      }
      if (ACptr->Qmax == ACptr->Max)
        strcpy(Qmax, "Qmax");
      else
        strcpy(Qmax, "Smax");
      if (ACptr->Qmin == ACptr->Min)
        strcpy(Qmin, "Qmin");
      else
        strcpy(Qmin, "Smin");
      if (QRcont && strpbrk(ACptr->Type, "G")) {
        if (Qlim && strpbrk(ACptr->cont, "V") &&
            (ACptr->Qg >= ACptr->Max || ACptr->Qg <= ACptr->Min)) {
          flag = true;
          Nvolt--;
          ...
          fprintf(
              stderr,
              "***Warning: Generator %d %s has lost remote voltage control\n",
              ACptr->Num, ACptr->Name);
          ...
          if (ACptr->Cont->Kbg < 1) {
            ...
            if (flagH)
              x0[ACvar[ACptr->Cont->N] + 1] = ACptr->Cont->V;
          ...
          fprintf(stderr, "***Warning: Generator %d %s has recovered\n",
                  ACptr->Num, ACptr->Name);
          fprintf(stderr, "            remote voltage control as Qg/Sg is "
                          "again within limits.\n");
          if (ACptr->Cont->Kbg == 1) {
            ...
            ACptr->Cont->Qr = ACptr->Qg / ACptr->Kbg;
            if (flagH)
              x0[ACvar[ACptr->Cont->N] + 1] = ACptr->Cont->Qr;
            fprintf(stderr, "***Warning: Remote voltage controlled bus %d %s\n",
                    ACptr->Cont->Num, ACptr->Cont->Name);
            fprintf(stderr, "            has recovered generator support.\n");
          }
        } else if (ACptr->Gen != nullptr) {
          if (Ilim && strpbrk(ACptr->cont, "V") &&
              ACptr->Gen->Ia >= ACptr->Gen->IaMax) {
            flag = true;
            Nvolt--;
            ACptr->Gen->Ia = ACptr->Gen->IaMax;
            ...
            fprintf(stderr, "***Warning: Generator %d %s has lost remote\n",
                    ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            V control due to Ia_max limit problems.\n");
            ACptr->Cont->Kbg--;
            if (ACptr->Cont->Kbg < 1) {
              ACptr->Cont->Cont = ACptr->Cont;
              ACptr->Cont->VCont = ACptr->Cont->V;
              if (flagH)
                x0[ACvar[ACptr->Cont->N] + 1] = ACptr->Cont->V;
              fprintf(stderr,
                      "***Warning: Remote voltage controlled bus %d %s\n",
                      ACptr->Cont->Num, ACptr->Cont->Name);
              fprintf(stderr, "            has lost all generator support.\n");
            }
          } else if (Ilim && Recover && strpbrk(ACptr->cont, "I") &&
            ...
            fprintf(
                stderr,
                "            remote V control as Ia is again within limits.\n");
            ACptr->Cont->Kbg++;
            if (ACptr->Cont->Kbg == 1) {
              ACptr->Cont->Cont = nullptr;
              ACptr->Cont->V = ACptr->Cont->VCont;
              if (flagH)
                x0[ACvar[ACptr->Cont->N] + 1] = ACptr->Qg / ACptr->Kbg;
              fprintf(stderr,
                      "***Warning: Remote voltage controlled bus %d %s\n",
                      ACptr->Cont->Num, ACptr->Cont->Name);
              fprintf(stderr, "            has recovered generator support.\n");
            ...
            fprintf(
                stderr,
                "            remote V control due to Eq_%3s limit problems.\n",
                str);
            ACptr->Cont->Kbg--;
            if (ACptr->Cont->Kbg < 1) {
              ACptr->Cont->Cont = ACptr->Cont;
            ...
            fprintf(
                stderr,
                "            remote V control as Eq is again within limits.\n");
            ACptr->Cont->Kbg++;
            if (ACptr->Cont->Kbg == 1) {
              ACptr->Cont->Cont = nullptr;
              ACptr->Cont->V = ACptr->Cont->VCont;
              ...
              fprintf(stderr, "            has recovered generator support.\n");
                     ...
                     ((Ilim && ACptr->Gen->Ia >= ACptr->Gen->IaMax) ||
                      (Elim && (ACptr->Gen->Eq >= ACptr->Gen->EqMax ||
                                ACptr->Gen->Eq <= ACptr->Gen->EqMin)))) {
            fprintf(stderr,
                    "***Error: Generator %d %s it's already at a Q limit.\n",
                    ACptr->Num, ACptr->Name);
            fprintf(stderr, "          Try changing Ia and/or Eq limits.\n");
            WriteSolution(0, TrueParamStr(2), "Ia/Eq Limit Problems:");
            exit(1);
        ...
        if (Qlim && strpbrk(ACptr->cont, "V") &&
          ...
          if (strpbrk(ACptr->Type, "G"))
            ACptr->Cont = ACptr->ContBus->AC;
          else
            ACptr->Cont = ACptr;
          if (flagH)
            x0[ACvar[ACptr->N] + 1] = ACptr->V;
          fprintf(stderr,
                  "***Warning: Generator %d %s has lost voltage control\n",
                  ACptr->Num, ACptr->Name);
          fprintf(stderr, "            due to %4s problems.\n", str);
        } else if (Qlim && Recover && strpbrk(ACptr->cont, "Q") &&
          ...
          fprintf(
              stderr,
              "            voltage control as Qg/Sg is again within limits.\n");
        } else if (ACptr->Gen != nullptr) {
          if (Ilim && strpbrk(ACptr->cont, "V") &&
              ACptr->Gen->Ia >= ACptr->Gen->IaMax) {
            flag = true;
            ...
            fprintf(stderr, "***Warning: Generator %d %s has recovered\n",
                    ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            V control as Ia is again within limits.\n");
          } else if (Elim && strpbrk(ACptr->cont, "V") &&
                     (ACptr->Gen->Eq >= ACptr->Gen->EqMax ||
                      ACptr->Gen->Eq <= ACptr->Gen->EqMin)) {
            flag = true;
            ...
            fprintf(stderr, "***Warning: Generator %d %s has recovered\n",
                    ACptr->Num, ACptr->Name);
            fprintf(stderr,
                    "            V control as Eq is again within limits.\n");
          } else if (strpbrk(ACptr->cont, "Q") &&
                     ((Ilim && ACptr->Gen->Ia >= ACptr->Gen->IaMax) ||
                      (Elim && (ACptr->Gen->Eq >= ACptr->Gen->EqMax ||
                                ACptr->Gen->Eq <= ACptr->Gen->EqMin)))) {
        ...
        if (Zlim && strpbrk(ACptr->cont, "V") &&
            (ACptr->Qg >= ACptr->Qmax || ACptr->Qg <= ACptr->Qmin)) {
          flag = true;
          NZvolt--;
          ...
          if (flagH)
            x0[ACvar[ACptr->N] + 1] = ACptr->V;
          fprintf(stderr,
                  "***Warning: Reactance-controlled bus %d %s has lost\n",
                  ACptr->Num, ACptr->Name);
          ...
          fprintf(stderr,
                  ...
                  ACptr->Num, ACptr->Name);
          fprintf(stderr,
                  "            voltage control as Q is again within limits.\n");
        }
      }
    }
  ...
  if ((Qlim || Elim || Ilim || Zlim) && RemoteVlost && QRcont)
    for (ACptr = dataPtr->ACbus; ACptr != nullptr; ACptr = ACptr->Next) {
      if (strpbrk(ACptr->Type, "C") && ACptr->Kg < 0)
        ACptr->Kbg = 0;
    }
  ...
  for (DCptrR = dataPtr->DCbus; DCptrR != nullptr; DCptrR = DCptrR->Next)
    if (!strcmp(DCptrR->Type, "R")) {
      ...
      if ((strcmp(DCptrR->Cont1, "AT") && strcmp(DCptrR->Cont2, "AT") &&
        ...
        flag = true;
        fprintf(stderr, "***Warning: The program will release tap limits in "
                        "the HVDC links.\n");
        break;
      }
    }
