...
/* Find PoC. */
{
  ...
  for (flagL = flags = true, M = 5, count = 0;;) {
    ...
    if (iter < 0) {
      iter = -iter;
      if (iter > MaxIter) {
        fprintf(stderr, "\n *** The PoC case has not been solved (possible "
                        "initial guess problems, or\n");
        fprintf(stderr, "     AC/DC/FACTS problems, or too few iterations). "
  ...
  if (count == M) {
    fprintf(stderr, "\n *** The PoC case has not been solved (possible initial "
                    "guess problems, or\n");
    fprintf(stderr, "     AC/DC/FACTS problems, or too few iterations). Try "
                    "running the case using\n");
    fprintf(stderr, "     the -F option, or change load levels, AC/DC "
                    "controls, or increase the\n");
