...
/* Detect inconsistencies in input data. */
{
  ...
  if (Ndc==0) dataPtr->DCbus=nullptr;
  if (NacEl==0) dataPtr->Element=nullptr;
  if (Nsvc==0) dataPtr->SVCbus=nullptr;         /* FACTS */
  if (Ntcsc==0) dataPtr->TCSCbus=nullptr;       /* FACTS */
  if (Nstatcom==0) dataPtr->STATCOMbus=nullptr; /* FACTS */
  if (Narea<2) {dataPtr->Area=nullptr; Narea=0;}
  ...
  while(ACptr!=nullptr){
    ...
    if (strpbrk(ACptr->Type,"G")){
      ...
      if (ACptr->Cont!=nullptr) {
        ...
        if (!QRcont) { ACptr->Cont->Cont=ACptr->Cont; ACptr->Cont=nullptr; }
        else {
          if (flag2Vcontrol) {
    ...
    if (Rcont && strpbrk(ACptr->Type,"T")) ACptr->Cont=nullptr;
    if (Narea>1){
      if(ACptr->Area==nullptr) for (Aptr=dataPtr->Area; Aptr!=nullptr; Aptr=Aptr->Next){
         for (i=1;i<=10;i++){
      ...
      if (ACptr->Area==nullptr) {
        ...
        if (ptrp!=nullptr) ptrp->Prev=Aptr->AC;
        if (ExistParameter('6') && Aptr->BSptr==nullptr) Aptr->Slack=Aptr->BSptr=ACptr;
      }
    } else ACptr->Area=nullptr;
 ...
 /* -------------------------- AC elements --------------------------- */
    ...
    if (strpbrk(ACptr->Type,"S,A")) {
      ...
      ptrs->Prev=nullptr;
      dataPtr->KGbus=ptrs;
      if (ptrp!=nullptr) ptrp->Prev=ptrs;
    }
    ACptr->PG=ACptr->Pg;
    ACptr->PL=ACptr->Pl;
    ACptr->QL=ACptr->Ql;
 ...
 /* ------------------- DC buses ------------------------ */
  for(DCptr=dataPtr->DCbus;DCptr!=nullptr;DCptr=DCptr->Next){
    ...
    if (DCptr->Xc==0) {
      ...
      Aptr=ACptr->Area;
      if (Aptr!=nullptr) {
        for (ptrp=Aptr->AC;ptrp!=nullptr;ptrp=ptrp->Next) if (ACptrp==ptrp->AC) break;
        if (ptrp!=nullptr) {
 ...
 /* ------------------- DC elements ------------------------ */
  ...
  for(DCptr=dataPtr->DCbus;DCptr!=nullptr;DCptr=DCptr->Next){
    ...
    if (DCptr->N!=0 && DCptrp->N!=0){
      ...
      if (DCptr->Area==DCptrp->Area||!strcmp(DCptr->Zone,DCptrp->Zone))   DCptr->Meter=DCptrp->Meter=nullptr;
