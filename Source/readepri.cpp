...
bool AddSection(ACbusData *From, ACbusData *To, char *Line, char *Ckt,
  ...
  for (ELptr = From->Elem; ELptr != nullptr; ELptr = ELptr->Next) {
    ...
    if (((From == Eptr->From && To == Eptr->To) ||
      ...
      if (Sec < Eptr->Sec) {
        Ge = Eptr->G;
        Eptr->G = G / (Eptr->Tap * Eptr->Tap);
        G = Ge;
        Be = Eptr->B;
        Eptr->B = B / (Eptr->Tap * Eptr->Tap);
        B = Be;
        Ge1 = Eptr->G1;
        Eptr->G1 = G1 / (Eptr->Tap * Eptr->Tap);
        G1 = Ge1;
        Be1 = Eptr->B1;
        Eptr->B1 = B1 / (Eptr->Tap * Eptr->Tap);
        B1 = Be1;
        Ge2 = Eptr->G2;
        Eptr->G2 = G2 / (Eptr->Tap * Eptr->Tap);
        G2 = Ge2;
        Be2 = Eptr->B2;
        Eptr->B2 = B2 / (Eptr->Tap * Eptr->Tap);
        B2 = Be2;
      } else {
...
/* Read Bus and Element data in WSCC format. */
{
  ...
  for (;;) { /* Reading Loop */
    ...
    LineNum++;
    if (!strncmp(Line, "C", 1) || !strncmp(Line, "BAS", 3))
      continue;
    ...
    /* --------------- AC bus data -------------------------------- */
      ...
      if (ACptr->PgMax < ACptr->Pg) {
        ACptr->PgMax = 99999999.;
        fprintf(stderr,
                "***Warning: Bus %d %s has its maximum generating power PgMax "
                "< Pg.\n",
                ACptr->N, ACptr->Name);
   ...
   the data set.  */
    ...
    /* --------------- X data cards -------------------------------- */
      ...
      if (!strcmp(ACptrp->Type, "B")) {
        ...
        fprintf(stderr, "Input Line-> %d\n%s", LineNum, Line);
        fprintf(stderr,
                "***Warning: This bus has been previously defined as a type %s "
                "bus, and hence\n",
                ACptrp->Type);
        fprintf(stderr, "            it cannot be defined as a BX remote bus. "
                        "The BX data will be ignored.\n");
      }
    }
    ...
    /* --------------- AC element data -------------------------------- */
      ...
      if (!flag) {
        ...
        if (!strncmp(Line, "L  ", 3))
          ...
          strcpy(Eptr->Type, "E");
        else if (!strncmp(Line, "T  ", 3) && strncmp(Eptr->Type, "R", 1))
          strcpy(Eptr->Type, "T");
      }
    }
    ...
    /* ------------------------ Area data -------------------------- */
      ...
      for (i = 36, j = 1; i <= 63; i = i + 3, j++)
        GetStr(Line, i, 2, 2, Aptr->Zone[j]);
    }
