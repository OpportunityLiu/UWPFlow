      ...
      if (Line[0] != 'C') {
        ...
        if (count == 8) {
          ...
          if (ACptr != nullptr) {
            if (strpbrk(ACptr->Type, "G,Q,E,S,V") == nullptr || Xd <= 0) {
              ...
              if (Xd <= 0)
                ...
                fprintf(stderr, "            The bus is not a generator type "
                                "(BQ,BG,BE,BV,BS).\n");
            } else {
          ...
          if (IaMax!=0 && strpbrk(ACptr->Type,"S")) {
             fprintf(stderr,"***Warning: The IaMax limit in slack
          generator:%s\n",ACptr->Name); fprintf(stderr,"            will be
          assumed large.\n"); IaMax=9999999999.;
          }
          */
              if (IaMax <= 0) {
  ...
  if (ExistParameter('v')) {
    ...
    if (V > 0.001) {
      ...
      if (ACptr != nullptr) {
        ...
        fprintf(stderr, "***Warning: The program will ignore the -v option "
                        "(there is no PQ bus).\n");
      }
    } else {
      fprintf(stderr,
