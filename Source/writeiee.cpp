...
void Print(FILE *File, int spaces, int width, int decimals, VALUETYPE val) {
  ...
  if (decimals)
    cons = 5 * pow(10., (double)-width - 1);
  else
    cons = 5 * pow(10., (double)-width);
  if (val < cons) {
    if (flagNegative && width > 1)
      width++;
    if (decimals)
      width++;
    if (width == 1)
  ...
  if (val < (pow(10., (double)width) - 0.5)) {
    ...
    j = 0;
    for (i = width - 1; i >= 0; i--) {
      if (val > (pow(10., (double)i) - 0.5 * pow(10., (double)-j)))
        break;
      j++;
    }
    if (decimals)
...
void IEEE(void) {
  ...
  if (card)
    fprintf(OutFile, "CARD\n");
  else
    fprintf(OutFile, "TAPE\n");
  t = time(nullptr);
  localt = localtime(&t);
  if (localt->tm_mday < 10)
    fprintf(OutFile, " 0%1d/", localt->tm_mday);
  else
    fprintf(OutFile, " %2d/", localt->tm_mday);
  month = localt->tm_mon + 1;
  if (month < 10)
    fprintf(OutFile, "0%1d/", month);
  else
    fprintf(OutFile, "%2d/", month);
  year = localt->tm_year;
  yearp = year + 1900;
  if (year >= 100)
    year = year - 100;
  if (year < 10)
    fprintf(OutFile, "0%1d", year);
  else
  ...
  for (ACptr = dataPtr->ACbus; ACptr != nullptr; ACptr = ACptr->Next) {
    if (ACptr->Num <= 9999)
      sprintf(Num, "%4d", ACptr->Num);
    else
      strcpy(Num, "****");
    if (Narea < 2 || ACptr->Area == nullptr)
      strcpy(Area, "0");
    else if (ACptr->Area->N <= 99)
    ...
    if (strpbrk(ACptr->Type, "G") && ACptr->Cont != nullptr)
      Print(OutFile, 1, 6, 4, ACptr->Cont->VCont);
    else if (strpbrk(ACptr->Type, "Q,S"))
      Print(OutFile, 1, 6, 4, ACptr->VCont);
    else
      Print(OutFile, 1, 6, 4, ACptr->V);
    ...
    for (ELptr = ACptr->Elem; ELptr != nullptr; ELptr = ELptr->Next) {
      ...
      if (Eptr->From == ACptr) {
        ...
        if (Eptr->B1 != Eptr->B2)
          ACptr->B = ACptr->B + Eptr->B1 * Eptr->Tap * Eptr->Tap;
      } else {
        ACptr->G = ACptr->G + Eptr->G2;
        if (Eptr->B1 != Eptr->B2)
          ACptr->B = ACptr->B + Eptr->B2;
      }
    }
    Print(OutFile, 1, 7, 4, ACptr->G);
    Print(OutFile, 1, 7, 4, ACptr->B);
    if (strpbrk(ACptr->Type, "G") && ACptr->Cont != nullptr) {
      if (ACptr->Cont->Num <= 9999)
        sprintf(Num, "%4d", ACptr->Cont->Num);
      else
        strcpy(Num, "****");
    } else
      strcpy(Num, "   0");
  ...
  for (Eptr = dataPtr->Element; Eptr != nullptr; Eptr = Eptr->Next) {
    if (Narea > 1 && Eptr->From->Area != Eptr->To->Area)
      Nties++;
    if (Eptr->From->Num <= 9999)
      sprintf(Num, "%4d", Eptr->From->Num);
    else
      strcpy(Num, "****");
    if (Eptr->To->Num <= 9999)
      sprintf(Nump, "%4d", Eptr->To->Num);
    else
      strcpy(Nump, "****");
    if (Eptr->Area == nullptr) {
      if (Eptr->From->Area == Eptr->To->Area)
        Eptr->Area = Eptr->From->Area;
      else
        Eptr->Area = Eptr->Meter->Area;
    }
    if (Narea < 2 || Eptr->Area == nullptr)
      strcpy(Area, "0");
    else if (Eptr->Area->N <= 99)
    ...
    if (strpbrk(Eptr->Type, "LE"))
      Type = 0;
    else if (strpbrk(Eptr->Type, "T"))
      Type = 1;
    else if (!strcmp(Eptr->Type, "R") || strpbrk(Eptr->Type, "V"))
      Type = 2;
    ...
    if (Eptr->B1 == Eptr->B2)
      Print(OutFile, 1, 9, 5, Eptr->B1 + Eptr->B2);
    else
      Print(OutFile, 1, 9, 5, 0.);
    Imax = ceil(Eptr->Imax * Sn);
    if (Imax > 99999)
      fprintf(OutFile, "99999");
    else
      fprintf(OutFile, " %5.0lf", Imax);
    fprintf(OutFile, "%12s", "");
    if (strpbrk(Eptr->Type, "R")) {
      if (Eptr->Cont->Num <= 9999)
        sprintf(Num, "%4d", Eptr->Cont->Num);
      else
        strcpy(Num, "****");
      ...
      if (strpbrk(Eptr->Type, "PQNM"))
        ...
        if (Eptr->Cont == Eptr->From || Eptr->Cont == Eptr->To)
          fprintf(OutFile, " 0 ");
        else
          fprintf(OutFile, " 1 ");
      }
    ...
    if (strpbrk(Eptr->Type, "RT")) {
      if (card)
        fprintf(OutFile, "\n");
      if (Eptr->Tap)
        Print(OutFile, 1, 6, 4, 1 / Eptr->Tap);
      else
  ...
  fprintf(OutFile, "-99\n");
  if (Narea > 1)
    fprintf(OutFile, "INTERCHANGE DATA FOLLOWS          %5d ITEMS\n", Narea);
  else
    fprintf(OutFile, "INTERCHANGE DATA FOLLOWS          %5d ITEMS\n", 0);
  if (Narea > 1)
    for (Aptr = dataPtr->Area; Aptr != nullptr; Aptr = Aptr->Next) {
      if (Aptr->N <= 99)
        sprintf(Area, "%2d", Aptr->N);
      else
        strcpy(Area, "**");
      if (Aptr->BSptr->Num <= 9999)
        sprintf(Num, "%4d", Aptr->BSptr->Num);
      else
        strcpy(Num, "****");
  ...
  for (DCptrR = dataPtr->DCbus; DCptrR != nullptr; DCptrR = DCptrR->Next) {
    ...
    if (!strcmp(DCptrR->Type, "R")) {
      ...
      for (i = 1; i <= 2; i++) {
        ...
        if (flag) {
          flag = false;
          if (!strcmp(DCptr->Cont1, "AL") || !strcmp(DCptr->Cont1, "GA"))
            strcpy(DCptr->Cont1, "AT");
          else if (!strcmp(DCptr->Cont2, "AL") || !strcmp(DCptr->Cont2, "GA"))
            strcpy(DCptr->Cont2, "AT");
          else
            strcpy(DCptr->Cont1, "AT");
        }
        ...
        for (j = 1; j <= 2; j++) {
          if (j == 1)
            strcpy(str, DCptr->Cont1);
          else
            strcpy(str, DCptr->Cont2);
          if (!strcmp(str, "VD"))
            Print(OutFile, 0, 6, 2, DCptr->Vd * Vn);
          if (!strcmp(str, "ID"))
            Print(OutFile, 0, 6, 2, DCptr->Id * In * 1000);
          if (!strcmp(str, "PA"))
            Print(OutFile, 0, 6, 2, fabs(DCptr->P) * Sn);
          if (!strcmp(str, "QA"))
            Print(OutFile, 0, 6, 2, fabs(DCptr->Q) * Sn);
          if (!strcmp(str, "AT"))
            Print(OutFile, 0, 6, 2, DCptr->Tap);
          if (!strcmp(str, "AL"))
            Print(OutFile, 0, 6, 2, DCptr->Alfa / K3);
          if (!strcmp(str, "GA"))
            Print(OutFile, 0, 6, 2, DCptr->Gamma / K3);
        }
        fprintf(OutFile, "%1s\n", DCptr->Type);
      }
  ...
  for (TCSCptr = dataPtr->TCSCbus; TCSCptr != nullptr;
    ...
    if (!strcmp(TCSCptr->Cont, "X"))
      Print(OutFile, 1, 7, 3, 100.0 / (TCSCptr->Bset * Max * Xc));
    else if (!strcmp(TCSCptr->Cont, "P"))
      Print(OutFile, 1, 7, 3, TCSCptr->Control * Sn);
    else if (!strcmp(TCSCptr->Cont, "I"))
      Print(OutFile, 1, 7, 3, TCSCptr->Control * Sn / Vn);
    else if (!strcmp(TCSCptr->Cont, "D"))
      Print(OutFile, 1, 7, 3, TCSCptr->Control / K3);
